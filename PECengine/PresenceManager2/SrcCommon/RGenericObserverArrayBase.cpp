...
void RGenericObserverArrayBase::NotifyObservers( MGenObserverNotifyMediatorBase& aNotifyMediator )
    ...
    for ( ii = 0; ii < Count(); ii++ )
        ...
        if ( obs )
            ...
            TRAPD( err, aNotifyMediator.MediateNotifyL( obs ) );
...
// -----------------------------------------------------------------------------
// RGenericObserverArrayBase::NotifyErrorObservers()
// -----------------------------------------------------------------------------
//
void RGenericObserverArrayBase::NotifyErrorObservers( MGenObserverNotifyMediatorBase& aNotifyMediator,
                                                      TInt aError )
    ...
    for ( ii = 0; ii < Count(); ii++ )
        ...
        if ( obs )
            {
            aNotifyMediator.MediateNotifyError( obs, aError );
            }
        }
...
// -----------------------------------------------------------------------------
// RGenericObserverArrayBase::IsNotifyLoopRunning()
// -----------------------------------------------------------------------------
//
TBool RGenericObserverArrayBase::IsNotifyLoopRunning() const
    {
    return iNotifyLoopRunning;
    }
...
void RGenericObserverArrayBase::FinalizeObsArrayAfterNotifyLoop()
    ...
    for ( TInt ii = ( RPointerArrayBase::Count() - 1 ); ii >= 0; ii-- )
        {
        if ( RPointerArrayBase::At( ii ) == NULL )  // CSI: 64 #
            {
            RPointerArrayBase::Remove( ii );
            }
        }
